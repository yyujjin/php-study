...
foreach ($planArr as $plan) {
    ...
    while ($startDay->format('Y-m') <= $endDay->format('Y-m')) {
        ...
        if (!in_array($startDay->format('Y-m'), $madeCalendar)) {
            ...
            $lastDate = date('w', strtotime("$year-$month-$lastDay")); //이거 이렇게 해줘야하나
...
//     //기간만큼 돌리기
//     //while ($startDay->format('Y-m') <= $endDay->format('Y-m')) {
//     while ($endDay->format('Y-m') >= $startDay->format('Y-m')) {

//         if (!in_array($endDay->format('Y-m'), $madeCalendar)) {

//             $madeCalendar[] = $endDay->format('Y-m');

//             //년, 달 빼기
//             $year = $endDay->format('Y');
//             $month = $endDay->format('n'); //일단 n으로 바꿈

//             //달력만들기 준비
//             //$day = 1;
//             $lastDay = $endDay->format('t');
//             $lastDate = date('w', strtotime("$year-$month-$lastDay")); //이거 이렇게 해줘야하나
//             $startDate = $endDay->format('w');
//             $totalWeek = ceil(($startDate + $lastDay) / 7);
//             $day = $lastDay;
